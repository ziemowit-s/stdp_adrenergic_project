...
if __name__ == '__main__':
    ...
    for i, (paradigm, label) in enumerate(zip(args.prefix, args.labels)):
        ...
        d, header = exclude_molecules(d, header=args.molecules, exact=['time', 'Ca', 'Leak'] + S845_merge.split(' '), wildcard=['out', 'buf'])
        for i in range(d.shape[0]):
            dataset.append((d[i], label, name))

    os.makedirs("cached", exist_ok=True)
    fname = "cached/regression_dataset_%s_%s_steps.pickle" % (args.morphology, args.num_steps)
